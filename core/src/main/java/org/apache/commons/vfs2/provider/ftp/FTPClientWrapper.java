...
class FTPClientWrapper implements FtpClient
{
    ...
    private FTPFile[] listFilesInDirectory(String relPath) throws IOException
    {
        ...
        {
            //One more check to make sure this directory really is empty
        	if(relPath != null && relPath.contains(" ") && (files == null || files.length == 0)) {
        	
	        	fileNames = getFtpClient().listNames(relPath);
	        	if (FTPReply.isPositiveCompletion(getFtpClient().getReplyCode()) && fileNames != null)
	        	{
	        		boolean realFileFound = false;
