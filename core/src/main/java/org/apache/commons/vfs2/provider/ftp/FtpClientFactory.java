        ...
        try
        {
            ...
            try
            {
                ...
                if (passiveMode != null && passiveMode.booleanValue())
                {
                	...
                	String activeExternalIPAddress = FtpFileSystemConfigBuilder.getInstance().getActiveExternalIPAddress(fileSystemOptions);
                	if(activeExternalIPAddress != null )
                	{
                		client.setActiveExternalIPAddress(activeExternalIPAddress);
                	}
                	
                	String reportActiveExternalIPAddress = FtpFileSystemConfigBuilder.getInstance().getReportActiveExternalIPAddress(fileSystemOptions);
                	if(reportActiveExternalIPAddress != null )
                	{
                		client.setReportActiveExternalIPAddress(reportActiveExternalIPAddress);
                	}
                	
                	Integer activePortRangeMin = FtpFileSystemConfigBuilder.getInstance().getActivePortRangeMin(fileSystemOptions);
                	Integer activePortRangeMax = FtpFileSystemConfigBuilder.getInstance().getActivePortRangeMax(fileSystemOptions);
                	
                	if(activePortRangeMin != null && activePortRangeMax != null && activePortRangeMin.compareTo(activePortRangeMax) <= 0)
                	{
                		client.setActivePortRange(activePortRangeMin.intValue(), activePortRangeMax.intValue());
                	}
