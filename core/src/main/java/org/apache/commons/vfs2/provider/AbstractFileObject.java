...
// Does not exist - make sure parent does
            ...
            if (parent != null)
            {
            	//Try added by Celigo. Fails for virtual folders. Folders should already exist
            	try {
            		parent.createFolder();
            	} catch (FileSystemException e) {
            		if (!(this instanceof FtpFileObject)) {
            			throw e;
            		}
            	}
            }
...
// Get the raw output stream
    ...
    /**
     * Prepares this file for writing. Doesn't check to see if the file exists
     *
     * @return An OutputStream where the new contents of the file can be written.
     * @throws FileSystemException if an error occurs.
     */
    public OutputStream getDirectOutputStream() throws FileSystemException
    {

    	// Get the raw output stream
        try
        {
            return doGetOutputStream(false);
        }
        catch (RuntimeException re)
        {
            throw re;
